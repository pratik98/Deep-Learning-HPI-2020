...
def get_metrics_for_LSTM(lstm_model,test_set):
    
    metric_val = lstm_model.evaluate(test_set)
...
#    Read the pre-trained model
    lstm_model = keras.models.load_model("lstm_saved_model.h5")
    
    # Printing Metrics for all the regions
    print("<=============================== Metrics ====================================>\r\n")
    fileObj = open("LSTM_Metrics.txt","w+")
    ...
    for each_region in forecast_regions:
      ...
      f_ds = load_file.create_window_dataset(f_series,10,shift_val=5)
      
      eval_metrics = get_metrics_for_LSTM(lstm_model,f_ds)
